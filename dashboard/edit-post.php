...
// Check post access before including header (to avoid output before redirect)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($content)) {
        ...
        if (isset($_FILES['featured_image']) && $_FILES['featured_image']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($ext, $allowed) && $file['size'] <= UPLOAD_MAX_SIZE) {
                ...
                if (move_uploaded_file($file['tmp_name'], $upload_path)) {
                    ...
                    if ($featured_image && file_exists(__DIR__ . '/../assets/images/' . $featured_image)) {
                        unlink(__DIR__ . '/../assets/images/' . $featured_image);
                    }
        ...
        $stmt = $conn->prepare("
            UPDATE posts 
            SET title = ?, slug = ?, excerpt = ?, content = ?, featured_image = ?, category_id = ?, status = ?, published_at = ?
    ...
    <form method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow-lg p-8">
        ...
        <div class="mb-6">
            ...
            <textarea id="excerpt" 
                      name="excerpt" 
                      rows="3"
                      class="w-full px-4 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500"><?php echo htmlspecialchars($post['excerpt'] ?? ''); ?></textarea>
        </div>
